(define (domain mintask)
  ...
  (:action completar-tarea-basica
    ...
    :effect (and (not (tBasica ?t)) (tPorRevisar ?t ?p) (increase (ttime) (+ (tiempo-tarea ?t) (calidad ?p))))
  )

  (:action completar-tarea-basica-con-suplemento
    :parameters (?t - Tarea ?p - Programador)
    :precondition (and (tBasica ?t) (= (+ (nivel-programador ?p) 1) (nivel-tarea ?t)) )
    :effect (and (not (tBasica ?t)) (tPorRevisar ?t ?p) (increase (ttime) (+ (+ (tiempo-tarea ?t) 2) (calidad ?p))))
  )

  (:action completar-tarea-por-revisar
    :parameters (?t - Tarea ?p2 ?p1 - Programador )
    :precondition (and (tPorRevisar ?t ?p2) (not (= ?p1 ?p2)) (>= (+ (nivel-programador ?p1) 1) (nivel-tarea ?t)) )
    :effect (and (not (tPorRevisar ?t ?p2)) (finalizada ?t) )
  )
